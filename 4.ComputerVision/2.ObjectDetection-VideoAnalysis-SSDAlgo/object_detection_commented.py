...
reader = imageio.get_reader('Deer.mov') # We open the video.
fps = reader.get_meta_data()['fps'] # We get the fps frequence (frames per second).
writer = imageio.get_writer('Deer_output.mp4', fps = fps) # We create an output video with this same fps frequence.
